...
for part in range(1, 6):
    ...
    with open(f"query_part_{part}.sql", "w") as f:
        json_data = json.dumps(questions).replace("'", "''")
        title = f"Droit du travail - Ultime (Partie {part})"
        f.write(f"DELETE FROM public.quizzes WHERE title = '{title}'; INSERT INTO public.quizzes (title, category, difficulty, questions) VALUES ('{title}', 'Droit du travail', 2, '{json_data}'::jsonb);")
